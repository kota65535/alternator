...
var Skipped = []TokenType{
	NewRegexpTokenType(-1, "\\s", false, 0, 0),
...
func TestPatternTokenType(t *testing.T) {
	...
	schema := "CREATE table `t1` (`id` int null)"

	l := NewLexer(strings.NewReader(schema),
		[]TokenType{
			NewSimpleTokenType(1, "CREATE", true, 1, 0),
			NewSimpleTokenType(2, "TABLE", true, 1, 0),
		},
		Skipped)
...
func TestRegexTokenType(t *testing.T) {
	...
	l := NewLexer(strings.NewReader(schema),
		[]TokenType{
			NewSimpleTokenType(1, "REFERENCES", true, 1, 0),
			NewRegexpTokenType(2, "[a-zA-Z0-9]+", true, 2, 0),
			NewRegexpTokenType(3, "MATCH (FULL|PARTIAL)", true, 1, 0),
		},
		Skipped)
...
func TestBalancedParenthesesTokenType(t *testing.T) {
	...
	l := NewLexer(strings.NewReader(schema),
		[]TokenType{
			NewRegexpTokenType(1, "[a-zA-Z0-9]+", false, 2, 0),
			NewRegexpTokenType(2, "`([a-zA-Z0-9]+?)`", false, 1, 0),
			NewSimpleTokenType(3, "(", false, 1, 0),
			NewSimpleTokenType(4, "CONSTRAINT", true, 1, 0),
			NewSimpleTokenType(5, "CHECK", true, 1, 0),
			NewBalancedParenthesesTokenType(6, ".*", '(', ')', false, 1, 5),
			NewSimpleTokenType(7, ")", false, 1, 0),
		},
		Skipped)
